...
with open('links.html','w') as f:
    ...
    for x in range(len(title)):
        card_code = f'<div class="card"><a target="_blank" rel="nofollow" href="{links[x]}"><div class="card-img" style="background-image: url({index_img[x]});"></div><div class="card-u"><div class="card-u-a"><img class="card-u-avatar" src="{avatar[x]}"></div><div class="card-u-t"><b class="card-u-t-title">{title[x]}</b><p class="card-u-t-text">{intro[x]}</p></div></div></a></div>'
